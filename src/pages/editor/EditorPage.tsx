...
export function EditorPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "生成项目失败。");
    } finally {
  ...
  const handleSaveVersion = async () => {
    ...
    setIsSaving(true);
    try {
      const nextProject = await projectService.createProjectVersion(project.id, "手动保存当前编辑版本");
      if (nextProject) {
        setProject(nextProject);
        toast.success("已保存新的版本快照。");
      }
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "保存项目版本失败。");
    } finally {
      setIsSaving(false);
    }
  };
    ...
    return (
      <PageTransition className="flex min-h-screen items-center justify-center bg-background px-6">
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <p className="text-sm text-muted-foreground">正在加载项目工作区...</p>
        </div>
      </PageTransition>
    );
  }
  ...
  if (projectId && loadError) {
    return (
      <PageTransition className="flex min-h-screen items-center justify-center bg-background px-6">
        ...
        >
          <h1 className="text-xl font-semibold text-foreground">加载项目失败</h1>
  ...
  if (projectId && !project) {
    return (
      <PageTransition className="flex min-h-screen items-center justify-center bg-background px-6">
        ...
        >
          <h1 className="text-xl font-semibold text-foreground">项目不存在</h1>
  ...
  return (
    <PageTransition className="flex min-h-screen flex-col bg-background">
          ...
          <AnimatePresence mode="popLayout">
            {messages.length === 0 ? (
              ...
              >
                ...
                <h2 className="mb-1 text-lg font-semibold text-foreground">你想做一个什么页面？</h2>
                <p className="mb-8 text-sm text-muted-foreground">输入你的需求，或直接选择下面的灵感模板。</p>
                  ...
                  {promptTemplates.map((template, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div>
                          <h3 className="mb-0.5 text-sm font-medium text-foreground">{template.label}</h3>
                          <p className="line-clamp-1 text-xs text-muted-foreground">{template.prompt}</p>
                        </div>
                      </button>
                    );
                  })}
                ...
                {messages.map((message) => (
                  ...
                  >
                    ...
                    <div className={`flex-1 ${message.role === "user" ? "text-right" : ""}`}>
                      ...
                      <p
                        className={`mt-1.5 px-2 text-[11px] text-muted-foreground/60 ${
      ...
      <div className="sticky bottom-0 z-20 border-t border-border bg-card/80 pb-[env(safe-area-inset-bottom)] backdrop-blur-xl">
