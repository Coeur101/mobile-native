...
- [x] 3.2 新增个人信息页并替换主页右上角入口，统一展示默认头像、默认昵称、邮箱与主题切换，收缩旧设置页职责，并完成对应 UI 自动化验证、临时测试清理、日志记录与独立 commit
- [x] 3.3 实现头像上传压缩后转 base64 存储到 Supabase profile，以及资料页内“设置密码 / 重置密码”的邮箱验证码确认流程，并完成对应功能验证、UI 自动化验证、临时测试清理、日志记录与独立 commit
